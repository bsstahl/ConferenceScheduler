    ...
    public static class SessionsExtensions
    {
        ...
        private static void VerifyPresenterAvailabilities(IEnumerable<Session> sessions)
        {
            ...
            foreach (var presenterId in presenterIds)
            {
                ...
                {
                    var presenterInstances = presenters.Where(p => p.Id == presenterId);
                    var baseAvailability = presenterInstances.First().UnavailableForTimeslots;
                    foreach (var presenter in presenterInstances)
                    {
        ...
        public static void ValidateAgainstRoomsAndTimeslots(this IEnumerable<Session> sessions, IEnumerable<Room> rooms, IEnumerable<Timeslot> timeslots)
        {
            ...
            if (roomSlotCombinations < sessions.Count())
                throw new ArgumentException($"You must have at least as many room-timeslot combinations ({roomSlotCombinations}) as you have sessions ({sessions.Count().ToString(System.Globalization.CultureInfo.CurrentCulture)}).");
